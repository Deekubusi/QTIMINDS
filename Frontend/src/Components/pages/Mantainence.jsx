...
const categoryIcon = (cat) => {
  ...
  switch (cat) {
    ...
    case "Housekeeping":
      return (
        <span
          className={`${base} inline-flex items-center justify-center text-emerald-500`}
        >
          🧹
        </span>
      );
    default:
      return (
        <span className={`${base} inline-block rounded-full bg-gray-400`} />
      );
  }
};
    ...
    setRequests((cur) =>
      cur.map((r) =>
        r.id === assigningId
          ? {
              ...r,
              assigned: assignName || "Unassigned",
              status: "In Progress",
            }
          : r
      )
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-yellow-200 bg-yellow-50 text-yellow-700">
              <span className="h-2 w-2 rounded-full bg-yellow-300" /> Low{" "}
              {alertCounts.low}
            </span>
          ...
          {filteredAlerts.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center py-10">
              ...
              <p className="text-sm text-gray-500">
                You’ll see new reminders here as they come in.
              </p>
            </div>
          ) : (
                  ...
                  return (
                    <tr key={r.id} className="border-t">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-medium ${badgeClasses(
                            r.status
                          )}`}
                        >
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${
                              statusDotClass[r.status] || "bg-gray-400"
                            }`}
                          />
                          {r.status}
                        </span>
                      ...
                      <td className="px-6 py-4">
                        {canRate ? (
                          <StarsEditable
                            onRate={(v) => setRequestRating(r.id, v)}
                          />
                        ) : (
...
function KPI({ title, value, hint, tone = "rose" }) {
  const bg =
    { rose: "bg-rose-100", amber: "bg-amber-100", emerald: "bg-emerald-100" }[
      tone
    ] || "bg-gray-100";
  return (
