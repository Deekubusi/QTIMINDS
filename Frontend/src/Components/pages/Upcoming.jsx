...
// Formats: "YYYY-MM-DD" | "D Mon YYYY" | already "dd/mm.yy"
function formatDateToDDMMYY(input) {
  if (!input) return "—";
  const trimmed = String(input).trim();

  if (/^\d{2}\/\d{2}\.\d{2}$/.test(trimmed)) return trimmed; // dd/mm.yy
...
// Outline-only pill (no background)
...
/** --- Modal --- */
const Modal = ({ title, open, onClose, children, onSubmit, submitText = "Save" }) => {
  if (!open) return null;
  return (
    ...
    <PayBadge value={row[3]} />
      ...
      <Modal
        ...
        onSubmit={(fd) => {
          ...
          const feedback = fd.get("feedback")?.toString() || "✅";
